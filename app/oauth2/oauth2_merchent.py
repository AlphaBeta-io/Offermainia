...
print(cursor)
oauth2_scheme = OAuth2PasswordBearer(tokenUrl = 'login/merchant')
...
def get_current_user(token:str = Depends(oauth2_scheme)):
    ...
    query = sql.SQL("SELECT id FROM {} AS adu WHERE adu.id = token.id").format(sql.Identifier('merchant_user'))
